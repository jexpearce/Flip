        ...
        ZStack {
            ...
            VStack(spacing: 8) {  // Reduced from 12 to 8 for more compact layout
                ...
                if let wasSuccessful = context.state.wasSuccessful, wasSuccessful {
                    SuccessLockView().transition(.opacity.combined(with: .scale)).id("success")  // Force view refresh when state changes
                }
                else if context.state.isFailed {
                    FailedLockView().transition(.opacity.combined(with: .scale)).id("failed")  // Force view refresh when state changes
                }
                else {
                    ActiveLockView(context: context).transition(.opacity.combined(with: .scale))
                        .id("active")  // Force view refresh when state changes
                }
            }
            .padding(8)  // Reduced from standard padding to 8
            .background(
                RoundedRectangle(cornerRadius: 15).fill(Color.white.opacity(0.05))  // Glass effect
            )
