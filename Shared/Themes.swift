...
struct Theme {
    ...
    static let profileGradient = LinearGradient(
        colors: [
            deepMidnightPurple, mediumMidnightPurple,
            darkCyanBlue.opacity(0.7),  // Using existing darkCyanBlue
...
extension Text {
    func title() -> Text {
        self.font(.system(size: 28, weight: .black)).tracking(8).foregroundColor(.white)
    }
    ...
    func retro() -> Text {
        self.font(.system(size: 60, weight: .black)).tracking(8).foregroundColor(.white)
    }
...
extension View {
    func glowingButton() -> some View {
        self.font(.system(size: 24, weight: .black)).tracking(4).foregroundColor(.white)
    ...
    func controlBackground() -> some View {
        self.padding(.vertical, 12).padding(.horizontal, 16)
            .background(
                ZStack {
                    RoundedRectangle(cornerRadius: 15).fill(Color.white.opacity(0.08))
