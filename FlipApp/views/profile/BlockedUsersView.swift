    ...
    var body: some View {
        ZStack {
            ...
            Circle().fill(cyanBlueAccent.opacity(0.1)).frame(width: 300).offset(x: 150, y: -200)
                .blur(radius: 60)
            Circle().fill(cyanBlueAccent.opacity(0.08)).frame(width: 250).offset(x: -150, y: 300)
                .blur(radius: 50)
            VStack(spacing: 20) {
                // Header
                HStack {
                    Text("BLOCKED USERS").font(.system(size: 28, weight: .black)).tracking(8)
                ...
                if isLoading {
                    ...
                    VStack(spacing: 15) {
                        ...
                        Text("No Blocked Users").font(.system(size: 20, weight: .bold))
                            .foregroundColor(.white)
                        Text("You haven't blocked any users yet").font(.system(size: 16))
        ...
        .alert(isPresented: $showUnblockAlert) {
            Alert(
                ...
                primaryButton: .destructive(Text("Unblock")) {
                    if let user = selectedUser { unblockUser(user) }
                },
...
struct BlockedUserRow: View {
    ...
    var body: some View {
        HStack(spacing: 15) {
            // Profile picture
            ProfileAvatarView(imageURL: user.profileImageURL, size: 50, username: user.username)
            // User info
            VStack(alignment: .leading, spacing: 4) {
                Text(user.username).font(.system(size: 16, weight: .bold)).foregroundColor(.white)
                Text("Blocked").font(.system(size: 14)).foregroundColor(.red.opacity(0.8))
            }
            Spacer()
            // Unblock button
            Button(action: onUnblock) {
                Text("Unblock").font(.system(size: 14, weight: .medium)).foregroundColor(.white)
                    .padding(.horizontal, 16).padding(.vertical, 8)
                    .background(
                        Capsule().fill(cyanBlueAccent.opacity(0.3))
                            .overlay(Capsule().stroke(cyanBlueAccent.opacity(0.5), lineWidth: 1))
                    )
            }
        }
        .padding()
