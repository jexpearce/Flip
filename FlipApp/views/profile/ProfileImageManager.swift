...
class ProfileImageManager: NSObject, ObservableObject {
    ...
    func uploadImage() {
        guard let currentUserId = Auth.auth().currentUser?.uid,
              let imageData = profileImage?.jpegData(compressionQuality: 0.7) else {
        ...
        let profileImagesRef = storage.child("profile_images/\(currentUserId)_\(Date().timeIntervalSince1970).jpg")
        
        // Upload the image
        let uploadTask = profileImagesRef.putData(imageData, metadata: nil) { [weak self] metadata, error in
