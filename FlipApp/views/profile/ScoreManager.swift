...
class ScoreManager: ObservableObject {
        ...
        if wasSuccessful {
            ...
            if !pausesEnabled {
                ...
                if streakMultiplier > 1.0 {
                    reason = "Completed \(actualDuration)min session (no pauses, \(String(format: "%.1fx", streakMultiplier)) streak bonus)"
                } else {
                    reason = "Completed \(actualDuration)min session (no pauses)"
                }
            } else {
                if streakMultiplier > 1.0 {
                    reason = "Completed \(actualDuration)min session (\(String(format: "%.1fx", streakMultiplier)) streak bonus)"
                } else {
    ...
    private func updateStreakStatus() {
        ...
        } else if currentStreak >= orangeStreakRequiredSessions && streakSessionsTime >= orangeStreakRequiredTime {
            streakStatus = .orangeFlame
        }
    }
    ...
    private func meetOrangeStreakCriteria() -> Bool {
        return currentStreak >= orangeStreakRequiredSessions && streakSessionsTime >= orangeStreakRequiredTime
    }
    ...
    private func loadStreakData() {
        ...
        db.collection("users").document(userId).collection("streak").document("current").getDocument { [weak self] document, error in
    ...
    private func saveStreakData(userId: String) {
        let streakData: [String: Any] = [
            ...
            "lastSuccessfulSession": lastSuccessfulSession.map { Timestamp(date: $0) } ?? FieldValue.serverTimestamp(),
            "updatedAt": FieldValue.serverTimestamp()
        ]
        
        db.collection("users").document(userId).collection("streak").document("current").setData(streakData) { error in
            if let error = error {
    ...
    func getStreakDescription() -> String {
        switch streakStatus {
        case .orangeFlame:
            return "🔥 ON FIRE! You're on a streak of \(currentStreak) successful sessions totaling \(streakSessionsTime) minutes."
        case .redFlame:
            return "🔥🔥 BLAZING! You're on an intense streak of \(currentStreak) successful sessions totaling \(streakSessionsTime) minutes!"
        case .none:
            if currentStreak > 0 {
