...
struct ZoomableProfileAvatar: View {
    ...
    var body: some View {
        Button(action: { withAnimation(.spring()) { showEnlarged = true } }) {
            ...
            ZStack {
                ...
                if let urlString = imageURL, !urlString.isEmpty, let url = URL(string: urlString) {
                    KFImage(url).placeholder { placeholderView }.cacheMemoryOnly()
                        .fade(duration: 0.25)
                        .setProcessor(
        ...
        .buttonStyle(ScaledButtonStyle()).onAppear { if streakStatus != .none { isGlowing = true } }
        .fullScreenCover(isPresented: $showEnlarged) {
            // Full screen enlarged view
            ZStack {
                // Dark background
                Color.black.opacity(0.9).edgesIgnoringSafeArea(.all)
                    .onTapGesture { withAnimation(.spring()) { showEnlarged = false } }
                ...
                ZStack {
                            ...
                            Circle()
                                .fill(
                                    streakStatus == .redFlame
                                        ? RadialGradient(
                                            gradient: Gradient(colors: [
                                                Color.red.opacity(0.5), Color.red.opacity(0.0),
                                            ]),
                                            center: .center,
                                            startRadius: 5,
                                            endRadius: 200
                                ...
                                .frame(width: 400, height: 400).scaleEffect(isGlowing ? 1.1 : 1.0)
                                .scaleEffect(scale).offset(dragAmount)

                            // Single large flame
                            Image(systemName: "flame.fill").font(.system(size: 200))
                                .foregroundColor(
                                    streakStatus == .redFlame
                                ...
                                .scaleEffect(isGlowing ? 1.05 : 0.95).scaleEffect(scale)
                                .offset(dragAmount)
                        }
                    }
                    ...
                    {
                        KFImage(url).placeholder { largePlaceholderView }.fade(duration: 0.25)
                            .resizable().aspectRatio(contentMode: .fit)
                            .frame(maxWidth: .infinity, maxHeight: .infinity).scaleEffect(scale)
                            .offset(dragAmount)
                            .gesture(
                                DragGesture().onChanged { value in dragAmount = value.translation }
                                    .onEnded { value in
                                        // Optional: Spring back to center if desired
                                        withAnimation(.spring()) {
                                            if scale <= 1.0 { dragAmount = .zero }
                                        }
                                    }
                            )
                            .gesture(
                                MagnificationGesture()
                                    .onChanged { value in scale = max(1.0, min(4.0, value)) }
                                    .onEnded { value in
                ...
                VStack {
                    HStack {
                        ...
                        Button(action: { withAnimation(.spring()) { showEnlarged = false } }) {
    ...
    private var largePlaceholderView: some View {
        ...
        return ZStack {
            Circle().fill(Theme.buttonGradient).frame(width: largeSize, height: largeSize)
                .opacity(0.2)

            if !username.isEmpty && username.count >= 1 {
                Text(String(username.prefix(1)).uppercased())
                    .font(.system(size: largeSize * 0.4, weight: .bold)).foregroundColor(.white)
            }
            else {
                Image(systemName: "person.circle.fill").font(.system(size: largeSize * 0.8))
                    .foregroundColor(.white)
            }
        }
        .shadow(color: Theme.lightTealBlue.opacity(0.5), radius: 15)
    }
}
