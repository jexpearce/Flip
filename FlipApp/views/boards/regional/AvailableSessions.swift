...
struct AvailableSessionsView: View {
    ...
    private var friendSessions: [LiveSessionManager.LiveSessionData] {
        ...
        return buildingSessions.filter { session in
            ...
            let isFriend =
                FirebaseManager.shared.currentUser?.friends.contains(session.starterId) ?? false
    ...
    private var otherSessions: [LiveSessionManager.LiveSessionData] {
        ...
        if userSettings.restrictLiveSessionsToFriends { return [] }
        return buildingSessions.filter { session in
        ...
        NavigationView {
            GeometryReader { geometry in
                ZStack {
                    ...
                    ZStack {
                        ...
                        Circle()
                            ...
                            .frame(width: 500, height: 500).offset(x: 150, y: -250)
                            .scaleEffect(pulsingEffect ? 1.1 : 1.0).blur(radius: 40)
                            .animation(
                        ...
                        Circle()
                            ...
                            .frame(width: 400, height: 400).offset(x: -150, y: 300).blur(radius: 40)
                    }
                    // Main content
                    VStack(spacing: 0) {
                        // Header with filter
                        HStack {
                            Button(action: { withAnimation { friendFiltered.toggle() } }) {
                                HStack(spacing: 6) {
                                    Text(friendFiltered ? "Showing Friends" : "All Sessions")
                                        .font(.system(size: 14, weight: .semibold))
                                        .foregroundColor(
                                            friendFiltered ? Theme.lightTealBlue : .white
                                        )
                                    Image(systemName: "chevron.down").font(.system(size: 12))
                                        .foregroundColor(
                                            friendFiltered
                                                ? Theme.lightTealBlue : .white.opacity(0.7)
                                        )
                                }
                        ...
                        HStack {
                            VStack(alignment: .leading, spacing: 2) {
                                Text("AVAILABLE SESSIONS").font(.system(size: 22, weight: .black))
                                    .tracking(2).foregroundColor(.white)
                                Text("Join a live session in this building").font(.system(size: 14))
                                    .foregroundColor(.white.opacity(0.7))
                            }
                            Spacer()
                            // Only show count badge if we have sessions
                            if (friendFiltered
                                ? friendSessions.count
                                : (friendSessions.count + otherSessions.count)) > 0
                            {
                                Text(
                                    "\(friendFiltered ? friendSessions.count : (friendSessions.count + otherSessions.count))"
                                )
                            ...
                            VStack(spacing: 25) {
                                ...
                                if userSettings.restrictLiveSessionsToFriends && !friendFiltered {
                                    VStack(spacing: 10) {
                                        ...
                                        Text(
                                            "You've chosen to only see sessions from friends. You can change this in Regional Privacy Settings."
                                        )
                                        .font(.system(size: 14))
                                        .foregroundColor(.white.opacity(0.7))
                                ...
                                // Empty state
                                if friendFiltered
                                    ? friendSessions.isEmpty
                                    : (friendSessions.isEmpty && otherSessions.isEmpty)
                                {
                                    VStack(spacing: 15) {
                ...
                .onAppear {
                    ...
                    withAnimation { pulsingEffect = true }
                }
            }
        }
    ...
    private func sessionSection(
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            HStack(spacing: 8) {
                ...
                Spacer()
                Text("\(sessions.count)").font(.system(size: 12, weight: .bold))
                    .foregroundColor(.white.opacity(0.7))
            }
...
struct EnhancedSessionCard: View {
        ...
        Button(action: {
            ...
            VStack(spacing: 0) {
                ...
                HStack(spacing: 12) {
                    ...
                    ZStack(alignment: .topTrailing) {
                        // Avatar circle
                        ZStack {
                            Circle().fill(getGradient()).frame(width: 48, height: 48)
                        ...
                        // Live indicator
                        ZStack {
                            Circle().fill(canJoin ? Color.green : Color.gray)
                                .frame(width: 14, height: 14)
                                .shadow(
                                    color: canJoin
                                        ? Color.green.opacity(0.6) : Color.gray.opacity(0.4),
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        Text(session.starterUsername).font(.system(size: 18, weight: .bold))
                        ...
                        HStack(spacing: 6) {
                            ...
                            Text("\(session.targetDuration)min").font(.system(size: 12))
                ...
                Rectangle()
                    .fill(
                        LinearGradient(
                            colors: [
                                .white.opacity(0.1), .white.opacity(0.2), .white.opacity(0.1),
                            ],
                            startPoint: .leading,
                            endPoint: .trailing
                        )
                ...
                HStack(spacing: 8) {
                    // Elapsed time
                    VStack(alignment: .leading, spacing: 2) {
                        Text("ELAPSED").font(.system(size: 10, weight: .medium)).tracking(1)
                            .foregroundColor(.white.opacity(0.7))
                        Text(formattedElapsedTime).font(.system(size: 18, weight: .bold))
                            .monospacedDigit().foregroundColor(.white).id(sessionTimer.currentTick)  // Force refresh
                    }
                    Spacer()
                    // Participant count
                    VStack(alignment: .trailing, spacing: 2) {
                        Text("PARTICIPANTS").font(.system(size: 10, weight: .medium)).tracking(1)
                            .foregroundColor(.white.opacity(0.7))
                        HStack(spacing: 4) {
                            Text("\(session.participants.count)")
                                .font(.system(size: 16, weight: .bold)).foregroundColor(.white)
                            Text(session.isFull ? "(FULL)" : "").font(.system(size: 12))
                ...
                HStack {
                    ...
                    Group {
                        if canJoin {
                            HStack(spacing: 8) {
                                Text("JOIN SESSION").font(.system(size: 14, weight: .bold))
                            ...
                            .shadow(
                                color: isFriend
                                    ? Color.green.opacity(0.3) : Theme.lightTealBlue.opacity(0.3),
                                radius: 4
                            )
                        }
                        else {
                            Text(isFull ? "SESSION FULL" : "UNAVAILABLE")
                                ...
                                .foregroundColor(.gray).padding(.horizontal, 16)
                                .padding(.vertical, 10)
                                .background(
            ...
            .background(
                ZStack {
                    ...
                    RoundedRectangle(cornerRadius: 16)
                        .strokeBorder(getCardBorderGradient(), lineWidth: 1.2)
                }
            )
            .shadow(color: getCardShadowColor(), radius: 8, x: 0, y: 4)
        }
        .buttonStyle(PlainButtonStyle()).scaleEffect(isPressed ? 0.98 : 1.0)
    ...
    private func getBackgroundColors() -> [Color] {
        if isFriend {
            return [
                Theme.forestGreen.opacity(0.4), Theme.darkBlue.opacity(0.3),
                Theme.deepMidnightPurple.opacity(0.3),
            ]
        }
        else {
            return [
                Theme.darkBlue.opacity(0.4), Theme.deepMidnightPurple.opacity(0.3),
    ...
    private func getCardBorderGradient() -> LinearGradient {
        if isFriend {
            return LinearGradient(
                colors: [
                    Color.green.opacity(isGlowing ? 0.7 : 0.4), Color.white.opacity(0.3),
            ...
            return LinearGradient(
                colors: [
                    Theme.lightTealBlue.opacity(isGlowing ? 0.7 : 0.4), Color.white.opacity(0.3),
    ...
    private func getCardShadowColor() -> Color {
        return isFriend ? Color.green.opacity(0.2) : Theme.lightTealBlue.opacity(0.2)
    }
}
