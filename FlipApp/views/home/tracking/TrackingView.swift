...
struct TrackingView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack {
                ...
                if appManager.isJoinedSession { JoinedSessionIndicator().padding(.bottom, 10) }
        ...
        .alert("Cancel Session?", isPresented: $showingCancelAlert) {
            Button("Cancel Session", role: .destructive) { appManager.failSession() }
        ...
        VStack(spacing: 20) {
            ...
            ZStack {
                ...
                Image(systemName: appManager.isFaceDown ? "iphone.gen3.circle.fill" : "iphone.gen3")
                    .font(.system(size: 60)).foregroundColor(appManager.isFaceDown ? .green : .red)
            ...
            if !appManager.isFaceDown && appManager.flipBackTimeRemaining > 0 {
                VStack(spacing: 8) {
                    Text("FLIP BACK OR PAUSE").font(.system(size: 16, weight: .bold))
                        .foregroundColor(Color.red).tracking(2)
                        ...
                        RoundedRectangle(cornerRadius: 10)
                            ...
                            .frame(
                                width: UIScreen.main.bounds.width * 0.7
                                    * CGFloat(appManager.flipBackTimeRemaining) / 10,
                                height: 10
                            )
                            .animation(
                                .linear(duration: 1),
                    ...
                    Text("\(appManager.flipBackTimeRemaining) seconds remaining")
                        .font(.system(size: 14, weight: .bold)).foregroundColor(.white.opacity(0.9))
                }
                ...
                .animation(
                    .easeInOut,
                    value: !appManager.isFaceDown && appManager.flipBackTimeRemaining > 0
                )
            }
        }
    ...
    private func timerCard() -> some View {
        VStack(spacing: 15) {
            Text("REMAINING").font(.system(size: 16, weight: .bold)).tracking(4)
            ...
            HStack(spacing: 10) {
                ...
                .frame(height: 80).padding(.horizontal, 30)
            }
        ...
        .padding(.vertical, 25).padding(.horizontal, 30)
        .background(
            ZStack {
                RoundedRectangle(cornerRadius: 20)
                    .fill(
    ...
    private func actionButtons() -> some View {
        VStack(spacing: 20) {
                ...
                && appManager.allowPauses && appManager.remainingPauses > 0
            {
                ...
                // Pause Button
                Button(action: {
                    withAnimation(.spring()) { isPausePressed = true }
                ...
                Button(action: {
                    ...
                    Text("CANCEL SESSION").font(.system(size: 16, weight: .bold)).tracking(2)
    ...
    private func limitedLocationWarning() -> some View {
        VStack {
            ...
            ZStack {
                ...
                RoundedRectangle(cornerRadius: 20).fill(Color.black.opacity(0.7)).blur(radius: 0.5)
                ...
                VStack(spacing: 10) {
                    HStack {
                        ...
                        Text("LIMITED LOCATION MODE").font(.system(size: 16, weight: .bold))
                            .tracking(1).foregroundColor(Theme.yellow)
                    }

                    Text(
                        "Keep your phone on during the entire session. You can use the pause button anytime."
                    )
                    .font(.system(size: 14)).multilineTextAlignment(.center).foregroundColor(.white)
                    ...
                    Button(action: {
                        ...
                        Text("GOT IT").font(.system(size: 14, weight: .bold))
                            .foregroundColor(.white).padding(.vertical, 8).padding(.horizontal, 20)
                            .background(
                                RoundedRectangle(cornerRadius: 15).fill(Theme.yellow.opacity(0.3))
            ...
            .frame(maxWidth: .infinity, maxHeight: 180).padding(.horizontal, 20)
            .padding(.bottom, 30)
        }
        .transition(.move(edge: .bottom).combined(with: .opacity))
        .animation(
            .spring(),
            value: appManager.usingLimitedLocationPermission && appManager.isFaceDown
        )
    }
}
