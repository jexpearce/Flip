...
struct AuthView: View {
    ...
    var body: some View {
        ZStack {
            ...
            // Success Overlay
            if showSuccessOverlay { successOverlayView }
    ...
    private var backgroundView: some View {
        ZStack {
            // Background gradient
            Theme.indigoPurpleGradient.edgesIgnoringSafeArea(.all)
    ...
    private var logoSection: some View {
        VStack(spacing: 0) {
            ...
            // Animated arrow icon
            Image(systemName: "arrow.2.squarepath").font(.system(size: 50, weight: .bold))
                .foregroundColor(.white).shadow(color: Theme.indigoGlow, radius: 6)
        ...
        .scaleEffect(isKeyboardVisible ? 0.8 : 1.0).padding(.top, isKeyboardVisible ? 20 : 60)
    }
    ...
    private var passwordField: some View {
        HStack(spacing: 15) {
            Image(systemName: "lock.fill")
                ...
                .frame(width: 20).scaleEffect(selectedField == .password ? 1.1 : 1.0)
            ...
            Group {
                if showPassword {
                    TextField(
                        "",
                        text: $password,
                        prompt: Text("Password").foregroundColor(.white.opacity(0.4))
                    )
        ...
        .shadow(color: selectedField == .password ? Theme.indigoGlow : .clear, radius: 4)
        .onTapGesture { selectedField = .password }.animation(.spring(), value: selectedField)
    }
        ...
        Button(action: {
            ...
            ZStack {
                Text(isSignUp ? "Create Account" : "Sign In")
                    .font(.system(size: 24, weight: .black)).tracking(4).foregroundColor(.white)
            ...
            .background(
                ZStack {
                    ...
                    RoundedRectangle(cornerRadius: 30).fill(Color.white.opacity(0.1))

                    RoundedRectangle(cornerRadius: 30).stroke(Theme.silveryGradient, lineWidth: 1)
                }
            )
    ...
    private var socialSignInButtons: some View {
        VStack(spacing: 20) {
            ...
            Button(action: {
                authManager.signInWithGoogle { success in if !success { isLoading = false } }
            }) {
            ...
            .overlay(
                VStack(spacing: 20) {
                    ...
                    Image(systemName: "checkmark.circle.fill").font(.system(size: 56))
                        ...
                        .shadow(color: Color.green.opacity(0.5), radius: 10).padding(.top, 30)
                    ...
                    Text("Your account has been created successfully!")
                        .font(.system(size: 18, weight: .medium)).multilineTextAlignment(.center)
                        .foregroundColor(.white).padding(.horizontal, 30).padding(.vertical, 10)
