...
class CustomLocationHandler {
    ...
    func createCustomLocation(name: String, at coordinate: CLLocationCoordinate2D, completion: @escaping (BuildingInfo?, Error?) -> Void) {
        guard let userId = Auth.auth().currentUser?.uid else {
            completion(nil, NSError(domain: "CustomLocationError", code: 1, userInfo: [NSLocalizedDescriptionKey: "User not authenticated"]))
            return
        }
        ...
        let locationData: [String: Any] = [
            ...
            "createdBy": userId,
            "creatorName": FirebaseManager.shared.currentUser?.username ?? "User",
            "isCustom": true,
        ...
        db.collection("custom_locations").document(locationId).setData(locationData) { error in
            if let error = error {
    ...
    func getNearbyCustomLocations(coordinate: CLLocationCoordinate2D, radiusInMeters: Double, completion: @escaping ([BuildingInfo]) -> Void) {
    ...
    func incrementLocationUsage(locationId: String) {
        ...
        userLocationRef.getDocument { document, error in
            if let document = document, document.exists {
                ...
                self.db.collection("custom_locations").document(locationId).getDocument { document, error in
