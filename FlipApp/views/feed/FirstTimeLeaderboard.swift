...
class FirstTimeLeaderboardManager: ObservableObject {
    ...
    func saveFirstSession(
        userId: String, username: String, duration: Int, wasSuccessful: Bool,
    ...
    func fetchLeaderboardAroundUser(
        userId: String, range: Int = 3, completion: @escaping (Bool) -> Void
    ) {
        self.isLoading = true
        ...
        db.collection("first_sessions")
            ...
            .getDocument { [weak self] snapshot, error in
                ...
                guard let data = snapshot?.data(),
                    ...
                    print(
                        "No user data found in first_sessions or missing required fields"
                    )
                    self.isLoading = false
                    completion(false)
                    return
                ...
                self.db.collection("first_sessions").getDocuments {
                    [weak self] countSnapshot, countError in
                    guard let self = self else { return }
                    ...
                    self.db.collection("first_sessions")
                        ...
                        .getDocuments {
                            [weak self] higherSnapshot, higherError in
                            guard let self = self else { return }
                            ...
                            let higherCount =
                                higherSnapshot?.documents.count ?? 0
                            ...
                            self.fetchEntriesAroundRank(
                                userRank: userRank, range: range
                            ) { success in
                                self.isLoading = false
                                completion(success)
                            }
    ...
    private func fetchEntriesAroundRank(
        userRank: Int, range: Int, completion: @escaping (Bool) -> Void
    ) {
...
struct FirstTimeLeaderboardView: View {
    @ObservedObject private var leaderboardManager = FirstTimeLeaderboardManager
        .shared
    ...
    var body: some View {
        VStack(spacing: 15) {
            ...
            if leaderboardManager.isLoading {
                ...
                VStack(spacing: 0) {
                    ...
                    ForEach(
                        Array(
                            leaderboardManager.leaderboardEntries.enumerated()),
                        id: \.element.id
                    ) { index, entry in
                        LeaderboardRow(
                            entry: entry, index: index,
                            slideInUser: $slideInUser, glowUser: $glowUser
                        )
        ...
        .onAppear {
            ...
            leaderboardManager.fetchLeaderboardAroundUser(userId: userId) {
                success in
                if success {
    ...
    var body: some View {
        HStack {
            ...
            Text("#\(entry.rank)")
                .font(
                    .system(
                        size: 14, weight: entry.isCurrentUser ? .black : .bold)
                )
                .foregroundColor(
                    entry.isCurrentUser
                        ? Theme.yellow
            ...
            Text(entry.username)
                .font(
                    .system(
                        size: 14, weight: entry.isCurrentUser ? .black : .medium
                    )
                )
                .foregroundColor(
                    entry.isCurrentUser ? .white : .white.opacity(0.9)
                )
                .lineLimit(1)
                .frame(maxWidth: .infinity, alignment: .leading)
            ...
            HStack(alignment: .firstTextBaseline, spacing: 2) {
                Text("\(entry.duration)")
                    .font(
                        .system(
                            size: 16,
                            weight: entry.isCurrentUser ? .black : .bold))
