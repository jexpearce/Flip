...
class FirstTimeLeaderboardManager: ObservableObject {
    ...
    func isFirstSession(userId: String, completion: @escaping (Bool) -> Void) {
        db.collection("sessions").whereField("userId", isEqualTo: userId).limit(to: 1)
    ...
    func saveFirstSession(
        ...
        db.collection("first_sessions").document(userId)
            .getDocument { [weak self] document, error in
                ...
                self.db.collection("first_sessions").document(userId)
    ...
    private func recalculateRankings() {
        db.collection("first_sessions").order(by: "duration", descending: true)
            .getDocuments { snapshot, error in
    ...
    func fetchLeaderboardAroundUser(
        userId: String,
        range: Int = 3,
        completion: @escaping (Bool) -> Void
    ) {
        ...
        db.collection("first_sessions").document(userId)
            .getDocument { [weak self] snapshot, error in
                ...
                guard let data = snapshot?.data(), let duration = data["duration"] as? Int,
                    let username = data["username"] as? String,
                    let wasSuccessful = data["wasSuccessful"] as? Bool
                else {
                    ...
                    .getDocuments { [weak self] countSnapshot, countError in
                        ...
                        self.db.collection("first_sessions")
                            ...
                            .getDocuments { [weak self] higherSnapshot, higherError in
                                ...
                                self.fetchEntriesAroundRank(userRank: userRank, range: range) {
                                    success in
                                    self.isLoading = false
                                    completion(success)
                                }
    ...
    private func fetchEntriesAroundRank(
        userRank: Int,
        range: Int,
        completion: @escaping (Bool) -> Void
    ) {
        ...
        db.collection("first_sessions").order(by: "duration", descending: true)
            .getDocuments { [weak self] snapshot, error in
                ...
                for (index, document) in documents.enumerated() {
                    ...
                    if isCurrentUser { userIndex = index }
                ...
                if userIndex >= 0 {
                    ...
                    print(
                        "Found user at index \(userIndex), showing entries \(startIndex) to \(endIndex)"
                    )
                }
                    ...
                    print(
                        "User not found in entries, showing top \(self.leaderboardEntries.count) entries"
                    )
                }
...
struct FirstTimeLeaderboardView: View {
    ...
    var body: some View {
        VStack(spacing: 15) {
            // Title
            VStack(spacing: 4) {
                Text("FIRST SESSION ACHIEVEMENT").font(.system(size: 16, weight: .bold)).tracking(2)
                    .foregroundColor(Theme.yellow)

                Text("GLOBAL FIRST FLIP LEADERBOARD").font(.system(size: 14, weight: .bold))
                    .tracking(2).foregroundColor(Color.white.opacity(0.7))
            }

            if leaderboardManager.isLoading {
                // Loading state
                VStack(spacing: 10) {
                    ProgressView().scaleEffect(1.5).tint(.white).padding(.bottom, 8)

                    Text("Ranking your achievement...").font(.system(size: 14, weight: .medium))
                ...
                // User's rank summary
                VStack(spacing: 8) {
                    Text("Your First Flip").font(.system(size: 18, weight: .bold))
                        .foregroundColor(.white)
                ...
                VStack(spacing: 0) {
                    // Column headers
                    HStack {
                        Text("RANK").frame(width: 50, alignment: .center)
                    ...
                    .foregroundColor(Color.white.opacity(0.6)).padding(.horizontal, 12)
                    .padding(.bottom, 8)
                    ...
                    // Leaderboard rows
                    ForEach(
                        Array(leaderboardManager.leaderboardEntries.enumerated()),
                        id: \.element.id
                    ) { index, entry in
                        LeaderboardRow(
                            entry: entry,
                            index: index,
                            slideInUser: $slideInUser,
                            glowUser: $glowUser
                        )
                        .opacity(showEntries ? 1 : 0).offset(y: showEntries ? 0 : 20)
                        .animation(
        ...
        .onAppear {
            ...
            leaderboardManager.fetchLeaderboardAroundUser(userId: userId) { success in
                if success {
                    ...
                    withAnimation(.easeOut(duration: 0.5)) { showEntries = true }
    ...
    var body: some View {
        HStack {
            ...
            Text("#\(entry.rank)")
                ...
                .foregroundColor(entry.isCurrentUser ? Theme.yellow : .white)
                .frame(width: 50, alignment: .center)

            // Username
            Text(entry.username)
                .font(.system(size: 14, weight: entry.isCurrentUser ? .black : .medium))
                .foregroundColor(entry.isCurrentUser ? .white : .white.opacity(0.9)).lineLimit(1)
            ...
            HStack(alignment: .firstTextBaseline, spacing: 2) {
                ...
                Text("min").font(.system(size: 12, weight: .medium))
                    .foregroundColor(
                        entry.isCurrentUser ? .white.opacity(0.9) : .white.opacity(0.7)
                    )
            }
            .foregroundColor(entry.isCurrentUser ? Theme.yellow : .white)
