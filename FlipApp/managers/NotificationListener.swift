...
class NotificationListener {
    ...
    func startListening() {
        ...
        UserDefaults.standard.set(lastNotificationTimestamp!.seconds, forKey: "lastNotificationTimestamp")
        ...
        notificationListener = db.collection("users")
            ...
            .addSnapshotListener { [weak self] snapshot, error in
                guard let snapshot = snapshot, error == nil else {
                    print("Error listening for notifications: \(error?.localizedDescription ?? "unknown error")")
                    return
                }
                ...
                for change in snapshot.documentChanges {
                    if change.type == .added {
                        ...
                        if let timestamp = data["timestamp"] as? Timestamp {
                            if newestTimestamp == nil || timestamp.seconds > newestTimestamp!.seconds {
                ...
                if let timestamp = newestTimestamp {
                    ...
                    UserDefaults.standard.set(timestamp.seconds, forKey: "lastNotificationTimestamp")
